...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      // Geist Mono can report a negative actualBoundingBoxLeft; sizing with left+right alone
      // then drawing at (pad - left) pushes trailing ink (F bars) past the canvas edge.
      const inkPad = Math.ceil(resolvedSize);
      const drawX = inkPad - left;
      const drawY = inkPad + ascent;
      offscreen.width = Math.ceil(Math.max(metrics.width, right - left) + drawX + inkPad);
      offscreen.height = Math.ceil(ascent + descent + inkPad * 2);
      offCtx.clearRect(0, 0, offscreen.width, offscreen.height);
      offCtx.font = font;
      offCtx.textAlign = "left";
      offCtx.textBaseline = "alphabetic";
      offCtx.fillStyle = "#ffffff";
      offCtx.fillText(content, drawX, drawY);

      const raw = offCtx.getImageData(0, 0, offscreen.width, offscreen.height);
      ...
      for (let y = 0; y < offscreen.height; y += 1) {
        for (let x = 0; x < offscreen.width; x += 1) {
          if (raw.data[(y * offscreen.width + x) * 4 + 3] > 20) {
            if (x < minX) minX = x;
            if (y < minY) minY = y;
            if (x > maxX) maxX = x;
            if (y > maxY) maxY = y;
      ...
      // Keep clear side margins so the last glyph never sits on the clip edge.
      const maxDrawWidth = Math.max(40, width * 0.82);
      const maxDrawHeight = Math.max(40, height * 0.72);
      const fitScale = Math.min(1, maxDrawWidth / inkWidth, maxDrawHeight / inkHeight);

      const targets: { x: number; y: number; alpha: number }[] = [];
      const step = Math.max(2, Math.floor(density));
      const originX = (width - inkWidth * fitScale) / 2;
      const originY = (height - inkHeight * fitScale) / 2;

      for (let y = minY; y <= maxY; y += step) {
        for (let x = minX; x <= maxX; x += step) {
          const alpha = raw.data[(y * offscreen.width + x) * 4 + 3];
          if (alpha > 40) {
            targets.push({
              x: originX + (x - minX) * fitScale,
              y: originY + (y - minY) * fitScale,
              alpha: alpha / 255,
            });
          }
        }
