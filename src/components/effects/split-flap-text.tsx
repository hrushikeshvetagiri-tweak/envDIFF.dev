...
type NamedCharset = keyof typeof CHARSETS;
...
const resolveCharset = (charset: string) => {
  if (charset in CHARSETS) return CHARSETS[charset as NamedCharset];
...
const SplitFlapText = ({
  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const updateTiles = (
        updates: { index: number; current: string; next: string; done: boolean }[]
      ) => {
        setTiles((previous) => {
